...
$sql_user = "SELECT * FROM tbl_order, tbl_user_account, tbl_order_detail, tbl_product WHERE tbl_user_account.id = '$khachhang_id' AND tbl_order.order_status = '0' AND tbl_order.user_id = tbl_user_account.id AND tbl_order_detail.order_code = tbl_order.order_code AND tbl_order_detail.product_id = tbl_product.id";
$query = mysqli_query($con, $sql_user);
$row = mysqli_fetch_array($query);
$o_c = $row['order_code'];
$sql_cart = "SELECT * FROM tbl_product, tbl_order_detail WHERE tbl_order_detail.order_code = '$o_c' AND tbl_order_detail.product_id = tbl_product.id";
